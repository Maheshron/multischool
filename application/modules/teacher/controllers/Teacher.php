...
class Teacher extends MY_Controller {
    ...
    * ********************************************************** */
    private function _get_posted_teacher_data() {
        ...
        $items[] = 'account_holder';
        $items[] = 'account_account';
        $items[] = 'bank_name';
        $items[] = 'bank_branch';
        $items[] = 'bank_ifsc';
        $items[] = 'qualification';
        ...
        if ($_FILES['photo']['name']) {
         ...
         if ($_FILES['certificates']['name']) {
            $data['certificates'] = $this->_upload_certificates();
        }
    ...
    * @return          : $return_photo string value 
    * ********************************************************** */
    private function _upload_certificates() {

        $prev_certificates = $this->input->post('prev_certificates');
        $photo = $_FILES['certificates']['name'];
        $photo_type = $_FILES['certificates']['type'];
        $return_photo = '';
        if ($photo != "") {
            if ($photo_type == 'image/jpeg' || $photo_type == 'image/pjpeg' ||
                    $photo_type == 'image/jpg' || $photo_type == 'image/png' ||
                ...
                $destination = 'assets/uploads/teacher-certificates/';

                $file_type = explode(".", $photo);
                $extension = strtolower($file_type[count($file_type) - 1]);
                $certificates_path = 'certificates-' . time() . '-sms.' . $extension;

                move_uploaded_file($_FILES['certificates']['tmp_name'], $destination . $certificates_path);

                // need to unlink previous photo
                if ($prev_certificates != "") {
                    if (file_exists($destination . $prev_certificates)) {
                        @unlink($destination . $prev_certificates);
                    }
                }

                $return_certificates = $certificates_path;
            }
        } else {
            $return_certificates = $prev_certificates;
        }
